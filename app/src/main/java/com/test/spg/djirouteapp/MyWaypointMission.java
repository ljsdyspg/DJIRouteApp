...
public class MyWaypointMission extends FragmentActivity implements TextureView.SurfaceTextureListener,View.OnClickListener{
    ...
    private Button locate, test, fpv, start, stop, resume, pause, clear, config;
    ...
    private void myWaypointMisson(){
        ...
        for (int i = 0; i < coords.size(); i++) {
            ...
            //添加到飞到每个点的动作，转云台，拍照
            List<WaypointAction> waypointActions = new ArrayList<WaypointAction>();
            waypointActions.add(new WaypointAction(WaypointActionType.GIMBAL_PITCH,-90));
            waypointActions.add(new WaypointAction(WaypointActionType.STAY,2000));
            waypointActions.add(new WaypointAction(WaypointActionType.START_TAKE_PHOTO,3000));

            mWaypoint.waypointActions = waypointActions;
    ...
    private void showSimpleSettingDialog() {
        /* @setView 装入自定义View ==> R.layout.dialog_customize
         * 由于dialog_customize.xml只放置了一个EditView，因此和图8一样
         * dialog_customize.xml可自定义更复杂的View
         */
        final View dialogView = LayoutInflater.from(MyWaypointMission.this)
                .inflate(R.layout.dialog_simplesetting,null);

        AlertDialog.Builder customizeDialog =
                new AlertDialog.Builder(MyWaypointMission.this)
                        .setView(dialogView)
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                // 获取EditView中的输入内容
                                EditText edit_altitude = (EditText) dialogView.findViewById(R.id.altitude);
                                EditText edit_speed = (EditText) dialogView.findViewById(R.id.speed);
                                float alt,spd;
                                alt = Integer.parseInt(nulltoIntegerDefalt(edit_altitude.getText().toString()));
                                spd = Integer.parseInt(nulltoIntegerDefalt(edit_speed.getText().toString()));
                                if (alt!=0&&spd!=0){
                                    altitude = alt;
                                    maxSpeed = spd;
                                    mSpeed = spd;
